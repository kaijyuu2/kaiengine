...
class BaseButton(Container):
    ...
    def updateChildrenLayers(self, lastlayer = None):
        ...
        highlight = self.getHighlight()
        if highlight:
            self.getHighlight().setLayer(base_layer + 1)
        return base_layer + 1
...
class GraphicButton(BaseButton):
    ...
    def __init__(self, *args, central_graphic = None, **kwargs):
        ...
        self.setWidth(self.getOrderedStylesheetData(DEFAULT_BUTTON_WIDTH, DEFAULT_WIDTH, default=self.getCentralWidget().getSpriteWidth()))
        self.setHeight(self.getOrderedStylesheetData(DEFAULT_BUTTON_HEIGHT, DEFAULT_HEIGHT, default=self.getCentralWidget().getSpriteHeight()))
